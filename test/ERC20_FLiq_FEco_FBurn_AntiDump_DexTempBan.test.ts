...
describe.only("UltimateCoin", function () {
  ...
  beforeEach(async function () {
    const SkyzaoFactory = <UltimateERC20__factory>(
      await ethers.getContractFactory("UltimateERC20")
    );
    contract = await SkyzaoFactory.deploy("Skyzao", "SKT");
    contract = await contract.deployed();
  });

  it("Should be constructed properly", async function () {
    expect(await contract.name()).to.equal("Skyzao");
    expect(await contract.symbol()).to.equal("SKT");
